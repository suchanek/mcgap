...
class MotorControl:
    ...
    def __init__(self, unit, port, speed, position, resolution):
        """
    ...
    def jogMotor(self, unit, delta, resolution):
        """
		Adjust motor delta.
    ...
    def readMotor(self, unit):
        ...
        #print "READ",unit,"LOC",location,"OFF"
        position = location
        #print "readMotor",unit,"Pos",position,location
...
class LocalIO:
    ...
    def chkPassword(self, root, pw):
        ...
        if pw.get() == 'abc123' or pw.get() == 'OOS39023':
            try:
                root.destroy()
                F.setConfig()
            except:
...
class TabControl:
    ...
    def setLabel2(self, t, position, x=205, y=145):
        global e1, e2, tk
        endpos = 7

        try:
            e1 = tk.Label(page[t], font=12, text=str(position))
            e1.place(x=x+20, y=y, width=100, height=20)
            #e2 = tk.Entry(page[t], font=12, textvariable=position, justify='right')
            #e2.delete(0, endpos)
            #e2.insert(0, position)
            #e2.place(x=x, y=y, width=70, height=20)
            # e1.delete(0, "end")
            # e1.insert(0, rv-offset)
        except:
            return
...
class MakeTab:
        ...
        for line in tab1:
            if line[1] != 'tab' and line[1] != 'res' and line[1] != 'ref':
                if len(line) == 5:
                    ...
                    name = line[3]
                butn="rb" + str(row)
                butn = tk.Radiobutton(page[1], font=20, text=name, command=partial(M1.setMotor, 1, 1), padx=20,
        ...
        for line in jog1:
            if line[1] == 'jog':
                if int(line[2]) < 0:
                    ...
                    row1 = row1 + 1
                if int(line[2]) > 0:
                    tk.Button(page[1], font=12, text=line[3], command=partial(M2.jogMotor, 1, line[2], 10), padx=40,
        ...
        for line in tab2:
            if line[1] != 'tab' and line[1] != 'res' and line[1] != 'ref':
                if len(line) == 5:
                    ...
                    name = line[3]
                tk.Radiobutton(page[2], font=20, text=name, command=partial(M2.setMotor, 2, 2), padx=20,
    ...
    def tablet3(self):
        ...
        for line in tab3:
            if line[1] != 'tab' and line[1] != 'res' and line[1] != 'ref':
                if len(line) == 5:
                    ...
                    name = line[3]
                tk.Radiobutton(page[3], font=20, text=name, command=partial(M3.setMotor, 3, 2), padx=20,
        ...
        for line in jog3:
            if line[1] == 'jog':
                if int(line[2]) < 0:
                    tk.Button(page[3], font=12, text=line[3], command=partial(M3.jogMotor, 3, line[2], 100), padx=40,
                          relief='ridge').place(x=380, y=jogS+30+row1*20, width=50, height=22)
                    row1 = row1 + 1
                if int(line[2]) > 0:
                    tk.Button(page[3], font=12, text=line[3], command=partial(M3.jogMotor, 3, line[2], 100), padx=40,
    ...
    def tablet4(seif):
        ...
        for line in tab4:
            if line[1] != 'tab' and line[1] != 'res' and line[1] != 'ref':
                if len(line) == 5:
                    ...
                    name = line[3]
                tk.Radiobutton(page[4], font=20, text=name, command=partial(M4.setMotor, 4, 2), padx=20,
        ...
        for line in jog4:
            if line[1] == 'jog':
                if int(line[2]) < 0:
                    tk.Button(page[4], font=12, text=line[3], command=partial(M4.jogMotor, 4, line[2], 100), padx=40,
                          relief='ridge').place(x=380, y=jogS+30+row1*20, width=50, height=22)
                    row1 = row1 + 1
                if int(line[2]) > 0:
                    tk.Button(page[4], font=12, text=line[3], command=partial(M4.jogMotor, 4, line[2], 100), padx=40,
    ...
    def warntab(seif, unit):
        ...
        tk.Label(page[unit], font=fontStyle, text=warn1, fg="#CC0000").place(x=60, y=50, width=350, height=25)
        tk.Label(page[unit], font=20, text=warn2).place(x=60, y=90, width=350, height=25)
        tk.Label(page[unit], font=20, text=warn3).place(x=60, y=120, width=350, height=25)
        tk.Label(page[unit], font=20, text=warn4).place(x=60, y=150, width=350, height=25)
        tk.Label(page[unit], font=20, text=warn5).place(x=60, y=180, width=350, height=25)
...
Motor check.
"""
M1 = MotorControl(1, 1, 1000, 0, 1)
M2 = MotorControl(2, 2, 1000, 0, 1)
M3 = MotorControl(3, 3, 1000, 0, 100)
M4 = MotorControl(4, 4, 1000, 0, 100)

if not M1.connected and not M2.connected and not M3.connected and not M4.connected:
        warn()
