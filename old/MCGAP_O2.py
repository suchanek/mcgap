...
class MotorControl:
    ...
    def checkLimits(self):
        ...
        if self.checkMotor():
            ...
            read = self.client.read_holding_registers(0x007D, 0x400, unit=self.unit)
            if isinstance(read, pymodbus.pdu.ExceptionResponse):
                print("!!! checkLimits: got modbus exception: ", ExceptionCodes.get(read.exception_code))
            else:
...
### NEED A DICTIONARY FOR NESSAGES
    ...
    def readDelay(self, target):
        ...
        while (rp != target):
            ...
            if ldelay == delay and reps == 5:
                if DBG:
                    print("!!! readDelay: Motor not going from",rp,"to",target)
                break
            
            if DBG:
                print(f"--- Wait {delay} sec to goto {target} from {rp} at {speed}")
    ...
    def sendMotor(self):
        ...
        self.position = rp
        if DBG:
            print(f"--- sendMotor {unit} {self.position} SPD {Speed[unit]}")
        return True
...
class TabControl:
    ...
    def getClosest(self, unit, position, tablist):
        ...
        if abs(difference) > 0 and unit < 3:
            ...
            tk.Label(page[unit], font='Ariel 13' , foreground="#F0F0F0", text=_warn1).place(x=50, y=240, width=450, height=25)
        #return closest
