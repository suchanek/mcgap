...
class MotorControl:
    ...
    def checkLimits(self):
        ...
        read = self.client.read_holding_registers(0x200F, 1, unit=self.unit)
        ...
        """
        # This throws an exception of 'Illegal Data Address'
        read = self.client.read_holding_registers(0x800F, 1, unit=self.unit)
        if read.isError():
            print("!!! checkLimits: got modbus exception: ", ExceptionCodes.get(read.exception_code))
        else:
    ...
    def connectMotor(self):
        ...
        checks for alarm status as well.
    ...
    def showAlarm(self, error):
        ...
        #b2 = tk.Button(w, text='Done', font='Ariel 13', width=30, command=temp.destroy, anchor=S)
        b.configure(width=10, activebackground="#BBBBBB")
    ...
    def readDelay(self, target):
        ...
        Updates self.position when done.
        ...
        while reps < maxTries and (rp != target or rp != READERROR):
            if DBG:
                print(f">>> readDelay: Unit {self.unit} attempt {reps + 1}")
        ...
        if rp == READERROR:
            print(f"readDelay: Unit {self.unit} error on reading position")
            return READERROR
        elif rp == target:
            self.position = rp
    ...
    def sendMotor(self, location):
        ...
        if DBG:
            print(f">>> sendMotor: Sending unit {self.unit}, to {self.target}")
    ...
    def setMotor(self, _tab):
        ...
        if self.unit == 1:
            location = [int(i[2]) for i in tmp1][slide.get()]
        if self.unit == 2:
            location = [int(i[2]) for i in tmp2][source.get()]
        if self.unit == 3:
            location = [int(i[2]) for i in tmp3][grate1.get()]
        if self.unit == 4:
            location = [int(i[2]) for i in tmp4][grate2.get()]
...
class InputControl:
    ...
    def convertL2S(self, lstr):
        """
        Converts first level list to a string.
        """
    ...
    def convertS2Dms(self, _val, res):
        """
        Converts a step value to degrees, minutes, seconds.
        """
    ...
    def convertS2Dcd(self, _val, res):
        """
        Converts a step value to decimal degrees.
        """
...
class LocalIO:
    ...
    def getPassword(self):
        ...
        b1 = tk.Button(pw, text="Enter", font='Ariel 13', command=partial(F.chkPassword, pswd, rs), pady=2, height=0, width=4, relief="groove")
    ...
    def setConfig(self):
        ...
        b1 = tk.Button(page[0], text="Save", font=6, command=partial(F.saveCfg, conf), pady=2, height=0, width=4, relief="groove")
        b2 = tk.Button(page[0], text="Cancel", font=6, command=partial(conf.destroy), padx=2, height=0, width=6, relief="groove")
...
class TabControl:
    ...
    def getRadioButn(self, unit):
        ...
        if unit == 1:
            ...
            closest = T.getClosest(unit, position, tmp1)
        if unit == 2:
            position = M2.readMotor()
            closest = T.getClosest(unit, position, tmp2)
        if unit == 3:
            position = M3.readMotor()
            closest = T.getClosest(unit, position, tmp3)
        if unit == 4:
            position = M4.readMotor()
            closest = T.getClosest(unit, position, tmp4)
            ...
            if line[1] == 'jog':
                if int(line[2]) < 0:
                    ...
                    tk.Button(page[1], font='Ariel 12', text=line[3], command=partial(M1.jogMotor, line[2]), padx=40,
                              relief="groove").place(x=380, y=jogS+30+row1*20, width=50, height=22)
                    row1 = row1 + 1
                if int(line[2]) > 0:
                    tk.Button(page[1], font='Ariel 12', text=line[3], command=partial(M1.jogMotor, line[2]), padx=40,
                              relief="groove").place(x=428, y=jogS+30+row2*20, width=50, height=22)
                    row2 = row2 + 1
